...
class OTWorker:
    ...
    async def process_message(self, message):
        ...
        async with message.process(requeue=True):
            ...
            for item in history_ops_data:
                ...
                if "ops" in item:
                    for hist_op in item["ops"]:
                        hist_op["op_type"] = hist_op.get(
                            "type", hist_op.get("op_type", "retain")
                        )
                        try:
            ...
            logger.info(
                f"Processed & Broadcasted OT for Doc: {doc_id} by User: {user_id} via RabbitMQ (Splitted into {len(ops_new)} ops)"
            )
...
async def main():
    ...
    exchange = await channel.declare_exchange(
        "ot_exchange", type="direct", durable=True
    )
    ...
    if worker_queues_str:
        queue_indexes = [
            int(q.strip()) for q in worker_queues_str.split(",") if q.strip().isdigit()
        ]
    else:
